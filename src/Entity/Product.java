...
public class Product {
  ...
  public static void create(Product product) throws SQLException {
    ...
    try {
      ...
      if (codeResultSet.next()) {
        System.out.println(
            ">>Entity.Product code Already exists!!! Please try with a unique Entity.Product code");
      } else {
        PreparedStatement productCreateStatement =
            productCreateConnection.prepareStatement(
                "INSERT INTO PRODUCT(CODE,NAME,unitcode,TYPE,PRICE,STOCK) VALUES (?,?,?,?,?,?)");
  ...
  public static void list() {
    ...
    try {
      ...
      ResultSet listresultSet =
          listStatement.executeQuery("SELECT * FROM PRODUCT ORDER BY ID LIMIT 20 ");
    ...
    if (count <= ((pageLength * pageNumber) - pageLength)) {
      ...
      try {
        ...
        PreparedStatement listStatement =
            listConnection.prepareStatement("SELECT * FROM PRODUCT WHERE ID BETWEEN ? AND ?");
    ...
    try {
      Statement listStatement =
          listConnection.createStatement(
              ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
      String listQuery =
          "SELECT * FROM PRODUCT WHERE " + attribute + " = '" + searchText + "'" + " ORDER BY ID";
      ResultSet listResultSet = listStatement.executeQuery(listQuery);
      if (listResultSet.next()) {
        listResultSet.beforeFirst();
        int pageCount = 1;
    ...
    try {
      ...
      if (listResultSet.next()) {
        ...
        if (rowCount > ((pageLength * pageNumber) - pageLength)) {
          ...
          System.out.println(">> Requested page doesnt exist !!!");
          System.out.println(
              ">> Existing page count with given pagination " + ((rowCount / pageLength) + 1));
        }
      } else {
        System.out.println(">> SearchText not Found ! Please try with an existing attribute value");
      }
  ...
  public static void edit(int id, String attribute, String value) throws SQLException {
    ...
    try {
      ...
      e.printStackTrace();
      System.out.println(
          ">> Template Mismatch! Please Try \"product edit help\" for proper attribute Constraints");
