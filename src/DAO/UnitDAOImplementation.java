...
public class UnitDAOImplementation implements UnitDAO {
  ...
  public Unit create(Unit unit)
    ...
    Unit unit1 = unitMapper.create(unit);
    sqlSession.commit();
    sqlSession.close();
    return unit1;
  }
  ...
  public List<Unit> list() throws ApplicationErrorException {
    ...
    //      ResultSet listResultSet = listStatement.executeQuery("SELECT * FROM UNIT ORDER BY
    // CODE");
  ...
  public int edit(Unit unit)
    ...
    //      unitConnection.setAutoCommit(false);
    //      String editQuery =
    //          "UPDATE UNIT SET NAME= COALESCE(?,NAME),CODE= COALESCE(?,CODE), DESCRIPTION=
    // COALESCE(?,DESCRIPTION),ISDIVIDABLE= COALESCE(?,ISDIVIDABLE) WHERE ID=?";
    ...
    //      if (e.getSQLState().equals("23505")) {
    //        throw new UniqueConstraintException(
    //            ">>Unit Code must be unique!!!\n The unit code you have entered already
    // exists!!!");
    //      } else {
    //        e.printStackTrace();
    //        throw new ApplicationErrorException(
    ...
    sqlSession = sqlSessionFactory.openSession();
    unitMapper = sqlSession.getMapper(UnitDAO.class);
    int status = unitMapper.edit(unit);
    sqlSession.commit();
    sqlSession.close();
    return status;
  }
  ...
  public int delete(String code) throws ApplicationErrorException {
    ...
    sqlSession = sqlSessionFactory.openSession();
    unitMapper = sqlSession.getMapper(UnitDAO.class);
    int rowsAffected = unitMapper.delete(code);
    sqlSession.commit();
    sqlSession.close();
    return rowsAffected;
  }
