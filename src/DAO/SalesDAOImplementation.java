  ...
  public List list(int pageLength, int pageNumber)
      throws ApplicationErrorException, PageCountOutOfBoundsException {
        ...
        if(count<=((pageLength*pageNumber)-pageLength)){
            ...
            try{
                ...
                PreparedStatement listSalesItemStatement=listConnection.prepareStatement ("SELECT P.NAME, S.PRODUCTCODE,S.QUANTITY,S.PRICE FROM SALESITEMS S INNER JOIN PRODUCT P ON P.CODE=S.PRODUCTCODE WHERE S.ID=?");
                List<SalesItem> salesItemList=new ArrayList<> ();
    ...
    public List list(String attribute, String searchText, int pageLength, int offset) throws ApplicationErrorException {
        ...
        try{
            ...
            String listQuery= "SELECT * FROM SALES WHERE "+attribute+" = '"+searchText+"' ORDER BY ID LIMIT "+ pageLength + " OFFSET "+offset;
    ...
    public int delete(int id) throws ApplicationErrorException {
        ...
        try{
            ...
            if(deleteStatement.executeUpdate ("DELETE FROM SALESITEMS WHERE ID='"+id+"'")>0 && deleteStatement.executeUpdate ("DELETE FROM SALES WHERE ID='"+id+"'")>0)
            {
                return 1;
            }
            else return -1;
