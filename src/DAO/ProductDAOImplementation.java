...
public class ProductDAOImplementation implements ProductDAO {
  ...
  public Product create(Product product)
      throws ApplicationErrorException, SQLException, UniqueNameException {
  ...
  public List<Product> list(String searchText) throws ApplicationErrorException {
    ...
    try {
      if (searchText.matches(codeRegex)) {
        ...
        Statement listStatement = listConnection.createStatement();
        String listQuery =
            "SELECT * FROM PRODUCT WHERE CAST(ID AS TEXT) ILIKE '"
                + searchText
                + "' OR CAST(PRICE AS TEXT) ILIKE '"
                + searchText
                + "' OR CAST(STOCK AS TEXT) ILIKE '"
                + searchText
                + "' OR CAST(COSTPRICE AS TEXT) ILIKE '"
                + searchText
                + "'";
        ResultSet listresultSet = listStatement.executeQuery(listQuery);
        while (listresultSet.next()) {
  ...
  public int delete(String parameter) throws ApplicationErrorException {
    ...
    try {
      ...
      } else if (parameter.matches(productCodeRegex)) {
        ResultSet stockResultSet =
  ...
  public int checkIsdividable(String code) throws ApplicationErrorException {
    ...
    try {
      Statement unitCheckStatement =
          unitCheckConnection.createStatement(
              ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
      ResultSet unitCheckResultSet =
          unitCheckStatement.executeQuery(
              "SELECT U.ISDIVIDABLE FROM PRODUCT P JOIN UNIT U ON P.UNITCODE=U.CODE WHERE P.CODE='"
                  + code
                  + "'");
